...
## RESULTADOS: FORECASTING (SERIES DE TIEMPO)

Se evaluó la capacidad de predecir el futuro (últimos 90 días del dataset) entrenando exclusivamente con el pasado, utilizando la variable `energia_diaria_kwh`. Previo al entrenamiento se realizó una imputación lineal para sanear el apagón del SCADA de 6 días.

**Modelos Competidores:**
1. **Holt-Winters (Suavizado Exponencial):** Modelo estadístico clásico (aditivo) con estacionalidad anual.
2. **Facebook Prophet:** Algoritmo moderno desarrollado por Meta, basado en series de Fourier.

**Desempeño en Test (Últimos 90 días):**

| Modelo | RMSE (kWh) | MAE (kWh) |
|---|---|---|
| Holt-Winters | 2438.92 | 1951.09 |
| Facebook Prophet | 1249.30 | 910.25 |

**Hallazgos y Conclusión:**
- **Ganador indiscutible: Facebook Prophet.** Logró reducir el error a la mitad en comparación con Holt-Winters. 
- La arquitectura de Prophet demostró ser muy superior para lidiar con el "ruido climático" (días nublados) que confunde a los modelos clásicos. Las series de Fourier logran capturar perfectamente la curva anual (verano vs invierno).
- Esta fase cierra el proyecto demostrando *verdadero poder de pronóstico* hacia el futuro, complementando el *nowcasting* (diagnóstico en tiempo real) logrado en la fase de Regresión Supervisada.
