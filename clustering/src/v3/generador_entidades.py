import numpy as np
import pandas as pd
from .clusters_v3 import CLUSTER_PROFILES_V3
...
def generar_entidades(n: int, clusters: np.ndarray, rng: np.random.Generator) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for i, c in enumerate(clusters):
        ...
        # REGLA DURA: Nuliparidad se evalúa primero
        nulliparous = int(rng.random() < p["nulliparous"])
        ...
        if nulliparous == 1:
            ...
            preterm_count = 0
            emergency_cesarean_count = 0
        else:
            parity_count = int(rng.integers(1, 6))
            delivery_count = int(rng.integers(0, parity_count + 1))
            ...
            # Nuevas variables historicas para clustering V3
            preterm_count = 0
            if delivery_count > 0 and rng.random() < p["preterm_prob"]:
                preterm_count = int(rng.integers(1, delivery_count + 1))
                
            emergency_cesarean_count = 0
            if cesarean_count > 0 and rng.random() < p["emergency_cesarean_prob"]:
                emergency_cesarean_count = int(rng.integers(1, cesarean_count + 1))
        ...
        medical_records.append({
            ...
            "emergency_cesarean_count": emergency_cesarean_count
        })
